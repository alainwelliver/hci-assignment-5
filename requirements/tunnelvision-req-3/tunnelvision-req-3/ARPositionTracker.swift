...
/// Wraps an ARKit world-tracking session. Publishes the camera's 3D position each frame.
/// Uses `.gravityAndHeading` alignment so the world axes map to compass directions:
///   +X = east, +Y = up, -Z = north.
@MainActor
final class ARPositionTracker: NSObject, ObservableObject {
    ...
    /// Called on the main thread every AR frame with the horizontal displacement (x = east, z = south).
